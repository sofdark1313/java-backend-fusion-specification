...
- 默认情况下,项目接口路径统一使用 `/api/**`,采用单一前台 Token 体系,无需刻意拆分前后台两套认证
- 只有项目明确存在独立后台管理端、需要在路径层就做隔离时,才同时启用 `/admin/**` 与 `/api/**`,并按 `必须` 拆出后台与前台两套独立 Token 体系
- 当项目使用 Spring Security 时,如果只有 `/api/**` 单体系,使用单一 `SecurityFilterChain` 即可;同时存在 `/admin/**` 与 `/api/**` 时,`推荐` 为不同用户体系拆分独立 `SecurityFilterChain`
- 匿名放行接口 `必须` 仅保留明确白名单,例如登录、注册、发送验证码、刷新令牌、公开能力接口
- `logout`、`me`、资料修改、业务查询等受保护接口 `必须` 要求有效 Access Token
- 同时存在多套用户体系时,Token `必须` 能在 `tokenType`、`issuer`、`audience`、命名空间或等价机制上完成隔离
- 同时存在多套用户体系时,缓存、黑名单、Refresh Token 命名空间 `推荐` 隔离
